...
def training_area(sel_prisma_date, legend, vector_LCZ_path):
    ...
    classes_LCZ.sort()
    print("List of testing samples LCZ classes: ", classes_LCZ)
...
def prisma_bbox(selected_prisma_image, sel_Sentinel2_date, output_dir):
    ...
    #output_dir = os.path.join(os.getcwd(), "study_area")
    ...
    bbox = os.path.join(output_dir, f"study_area_{sel_Sentinel2_date.replace('-', '')}.gpkg")
...
def clip_training_sample(image_path, training_path, sel_s2_date, study_area, roi_new_path):
...
def parameter_tuning(classification_method, X_train, y_train):
    ...
    if classification_method == 'Random Forest':
        ...
        param_grid = {
            'n_estimators': [200, 500],#[100, 150, 200, 500],
...
def export_classified_map(img, clc, X , selected_prisma_image, classification_method, sel_prisma_date, folder_path):
    ...
    #classified_file = os.path.join(folder_path, f'classified_{classification_method}_{sel_prisma_date.replace("-", "")}_20m.tif')
    #smoothed_file = os.path.join(folder_path, f'classified_{classification_method}_{sel_prisma_date.replace("-", "")}_medianfilter_20m.tif')

    # save the classified image with rasterio
    with rasterio.open(folder_path + '/classified_' + classification_method + '_' + sel_prisma_date.replace('-', '') + '_20m.tif', 'w', **kwargs) as dst:
        dst.write(class_prediction, 1)
        print(f"The classified file {'/classified_' + classification_method + '_' + sel_prisma_date.replace('-', '') + '_20m.tif'} has been created!")
    ...
    # save the classified image with rasterio
    with rasterio.open(folder_path + '/classified_' + classification_method + '_' + sel_prisma_date.replace('-', '') + '_medianfilter_20m.tif', 'w', **kwargs) as dst:
        dst.write(smoothed_image, 1)
        print(f"The smoothed classified file {'/classified_' + classification_method + '_' + sel_prisma_date.replace('-', '') + '_medianfilter_20m.tif'} has been created!")

def testing_area(sel_prisma_date, legend, vector_LCZ_path):
...
import matplotlib.colors as colors
def plot_classified_image(sel_prisma_date, classified_image, classification_method, legend):
    
    # Plot the raster data with a custom figure size
    fig = plt.figure(figsize = (14, 14))
    ax = fig.add_subplot(1, 1, 1)
    plt.title(f"Local Climate Zone map obtained with {classification_method} from PRISMA image of {sel_prisma_date}")

    #cmap = colors.ListedColormap([colors_dict[key] for key in colors_dict])
    cmap = colors.ListedColormap([legend[key][1] for key in legend.keys()])
    bounds = [int(key) for key in legend.keys()]
    bounds.append(bounds[-1]+1)
    norm = colors.BoundaryNorm(bounds, cmap.N)
    ...
    labels = [legend[key][0] for key in legend.keys()]
    #handles = [plt.Rectangle((0, 0), 1, 1, color = colors_dict[label]) for label in labels]
    handles = [plt.Rectangle((0, 0), 1, 1, color = legend[key][1]) for key in legend]
    legend = plt.legend(handles, labels, title = 'LCZ class', bbox_to_anchor = (1.05, 1), loc = 'upper left')
    plt.setp(legend.get_title(), fontsize = '12')  # Adjust the font size of the legend title
...
def print_accuracy_s2(classification_method, sel_s2_date, prisma_s2_dates, legend, classified_image_path, testing_set_path ):
    #with rasterio.open(f'./classified_images/comparison_PRISMA_S2/classified_{classification_method}_' + sel_s2_date.replace('-', '') + '_medianfilter_20m_align_S2_masked.tif') as src:
    ...
    with rasterio.open(testing_set_path ) as src:
        test = src.read()
